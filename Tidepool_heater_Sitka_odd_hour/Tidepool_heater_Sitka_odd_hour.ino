/*  Tidepool_heater_Sitka_odd_hour.ino
    Luke Miller 2019
    Heater output set to 60W, runs during odd hours at low tide 
    (see Tidepool_heater_Sitka_even_hour.ino for the even-hour version) 
  ...
  switch(mainState){
    case STATE_IDLE:
      ...
      if ( (tideHeightft < tideHeightThreshold) & 
        ...
        !lowVoltageFlag & 
        !lowtideLimitFlag & 
        ( (newtime.hour() % 2) != 0) ){ // hour%2!=0 is true when hour is odd
    ...
    case STATE_HEATING:
      ...
      if ( (tideHeightft < tideHeightThreshold) & 
         ( (newtime.hour() % 2) != 0) &  // Must be odd hour, hour%2 != 0 returns true if odd hour
        (newtime.hour() < sunsetHour) ) {
          ...
          mainState = STATE_IDLE;
          lowtideLimitFlag = false;
        } else if ( (newtime.hour() % 2) == 0){
          // If the new hour is an even number, turn off
          mainState = STATE_IDLE;
          lowtideLimitFlag = false;
        }
