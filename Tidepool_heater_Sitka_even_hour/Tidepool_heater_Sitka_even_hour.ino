/*  Tidepool_heater_Sitka_even_hour.ino
    Luke Miller 2019
    Heater output set to 60W, runs during even hours at low tide
    (see Tidepool_heater_Sitka_odd_hour.ino for the odd-hour version) 
...
void setup() {
  ...
  Serial.println(F(" ft."));
  Serial.println(F("Even hours on"));
  //*************************
  ...
  switch(mainState){
    case STATE_IDLE:
      ...
      if ( (tideHeightft < tideHeightThreshold) & 
        ...
        !lowVoltageFlag & 
        !lowtideLimitFlag &
        ( (newtime.hour() % 2) == 0) ){  // hour%2==0 is true when hour is even
    ...
    case STATE_HEATING:
      ...
      if ( (tideHeightft < tideHeightThreshold) & 
        ( (newtime.hour() % 2) == 0) &  // Must be even hour, returns true if hour%2==0
        (newtime.hour() < sunsetHour) ) {
          ...
          // If the new hour is not an even number, turn off
          mainState = STATE_IDLE;
          lowtideLimitFlag = false;
        }
